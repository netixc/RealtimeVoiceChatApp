...
LLM_START_MODEL = "openai/gpt-4o-mini"
NO_THINK = False
DIRECT_STREAM = False
...
from services.audio.audio_in import AudioInputProcessor
from services.speech.speech_pipeline_manager import SpeechPipelineManager
from utils.colors import Colors
...
app.mount("/static", NoCacheStaticFiles(directory="/app/src/static"), name="static")
...
async def favicon():
    ...
    """
    return FileResponse("/app/src/static/favicon.ico")
...
if __name__ == "__main__":
    ...
    if not USE_SSL:
        ...
        cert_file = os.getenv("SSL_CERT_PATH", "127.0.0.1+1.pem")
        key_file = os.getenv("SSL_KEY_PATH", "127.0.0.1+1-key.pem")
